...
async function printTopPersons(topPersons, filename) {
  const result = {
    topComposers: [],
    topPerformers: [],
  ...
  while (true) {
    ...
    for (const composerId in currentTopPersons.composers) {
      if (composerId in topPersons.composers) {
        topPersons.composers[composerId] += currentTopPersons.composers[composerId];
      } else {
        topPersons.composers[composerId] = currentTopPersons.composers[composerId];
      }
    }
    ...
    for (const performerId in currentTopPersons.performers) {
      if (performerId in topPersons.performers) {
        topPersons.performers[performerId] += currentTopPersons.performers[performerId];
      } else {
        topPersons.performers[performerId] = currentTopPersons.performers[performerId];
      }
    }
    ...
    for (const composerId in currentTopPersons.performersByComposer) {
      ...
      for (const performerId in currentTopPersons.performersByComposer[composerId]) {
        if (performerId in topPersons.performersByComposer[composerId]) {
          topPersons.performersByComposer[composerId][performerId] += currentTopPersons.performersByComposer[composerId][performerId];
        } else {
          topPersons.performersByComposer[composerId][performerId] = currentTopPersons.performersByComposer[composerId][performerId];
        }
      }
    ...
    if (eventKeys.length < batchSize) {
      console.log('Number of events in apiResponse.event is less than batchSize. Exiting the loop.');
      break;
    }
  }

  await printTopPersons(topPersons, 'statisticAutogenerated.json');
}
